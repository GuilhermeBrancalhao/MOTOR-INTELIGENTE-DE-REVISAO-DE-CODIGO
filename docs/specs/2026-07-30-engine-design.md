...
9. **Execução persistente (R10):** escrita cujo alvo instala código para rodar depois, sozinho — `.git/hooks/`, `.claude/`, `.vscode/`, `.idea/`, arquivos de inicialização de shell (`.bashrc`, `.zshrc`, `.profile`…), perfil do PowerShell (`*profile.ps1`), pasta `Startup` do Windows, `crontab`, `.gitconfig` e `authorized_keys`. Trava dentro **ou** fora da raiz do projeto. A família existe porque escrita de arquivo **novo** é livre por política — e um `Write` em `.git/hooks/pre-commit` ou em `.claude/settings.json` era exatamente isso: arquivo novo, liberado sem aparecer no relatório, instalando um comando arbitrário para executar a cada commit ou a cada sessão.
10. **Destruição de dados ou de infraestrutura (R11):** o que zera ou sobrescreve arquivo e volume **sem passar por um verbo de apagar**, e o que desregistra infraestrutura inteira — `truncate -s`, `dd of=`, `robocopy /MIR`, `cipher /w`, `format X:`, `wsl --unregister`, `reg delete /f`, `pip uninstall` e o truncamento por redirecionamento puro (`> arquivo` sem comando à esquerda). A família existe porque a deleção (item 3) casa os verbos de apagar, e nenhum destes usa um: `truncate -s 0 banco.db` destrói o arquivo tanto quanto `rm`, mas saía `rastreado` e executava.
11. **Comando grande demais (R12):** comando acima do teto de tamanho (20.000 caracteres) **trava**, sem ser analisado padrão a padrão — e travar é o lado certo do erro. Comando útil não tem dezenas de milhares de caracteres, e varrer cada família sobre um texto desse tamanho era justamente o vetor de negação de serviço por regex: um comando artificialmente repetitivo prendia o classificador — e a sessão inteira — por segundos. O humano confirma um comando anômalo em vez de a sessão congelar tentando entendê-lo.
