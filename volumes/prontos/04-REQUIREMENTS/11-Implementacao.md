...
<!-- exemplo: exemplos/04-requirements/requisito.py -->

`requisito.py`, citado acima, formaliza Q1-Q8 como estrutura de dado: `Requisito` recusa se
construir sem rastro para trás quando a origem não é humana (Q3), `Conjunto.retirar` aposenta o
id em vez de liberá-lo (Q4), e `completo()` exige as duas condições de Q2 e Q3 ao mesmo tempo —
zero pendência aberta **e** zero requisito sem verificação associada.
