...
<!-- exemplo: exemplos/02-core/fronteira.py -->

`fronteira.py`, citado acima, formaliza as oito regras (N1-N8) em código: a fronteira de saída
como função nomeável única (N1), as três camadas de validação na ordem que N3 exige, e a
exceção `SemEfeito` que torna N4 impossível de contornar por acidente — não existe caminho de
código que aplique efeito sem passar por uma `RespostaValidada` de verdade.
